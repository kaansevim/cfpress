...
//
// ORCID/kurum bilgisi OJS'te girilmemiş olabilir. Boş alan için satır ÇİZİLMEZ;
// aksi halde boş bir ORCID rozeti çıkar ve orcid.org anasayfasına giden
// anlamsız bir bağlantı oluşur.

function OrcidLink({ orcid, className = "" }: { orcid: string; className?: string }) {
  ...
  return (
    <a
      ...
      target="_blank"
      rel="noreferrer"
      className={`inline-flex items-center gap-1 font-mono text-xs text-muted-foreground hover:text-accent ${className}`}
    >
      <span
        aria-hidden
...
export function AuthorContributions({ authors }: { authors: Author[] }) {
  return (
    <div className="space-y-5">
      {authors.map((a, index) => (
        <div key={`${a.name}-${index}`} className="rounded-lg border border-border p-4">
          ...
          <OrcidLink orcid={a.orcid} className="mt-1" />
